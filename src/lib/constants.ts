...
const VARIATION_STRENGTH_PROMPTS: Record<VariationStrength, string> = {
  subtle: `变化强度：轻微。保持同一主题和同一艺术媒介，但重新安排主体、背景、元素密度、留白和装饰节奏；画面应明显不同，不能只是重绘原图。`,
  balanced: `变化强度：中等。必须保持主题相似和风格相似，但构图要大胆重构：改变视角或图案排布，替换部分元素组合，调整元素大小层级、密度、背景节奏和视觉焦点。结果应像同一系列的新设计，而不是原图的临摹、裁切、扩写或轻微改色。`,
  bold: `变化强度：大胆。不要编辑原图，也不要生成原图的同款变体；只提取原图的图片类型、题材类别、媒介风格和商业用途，然后直接生成一张同类型的新图片。允许替换大部分具体元素、重新选择取景、重新设计场景和构图。结果应像同类目素材库里的另一张新图，而不是原图的重绘、扩写、换色或同系列小改。`,
}
...
const VARIATION_DIRECTION_PROMPTS: Record<VariationStrength, string[]> = {
  ...
  bold: [
    `本张大胆方向：直接生成同类型的新素材，采用全新的俯视/平铺式构图；只保留题材类别和媒介风格，不保留原图元素位置。`,
    `本张大胆方向：直接生成同类型的新素材，采用近景细节取景，重新选择主体和背景；不要复用原图的主体比例、裁切和空间结构。`,
    `本张大胆方向：直接生成同类型的新素材，做更密集的满版构图，替换大部分具体元素，只保留同类题材和视觉风格。`,
    `本张大胆方向：直接生成同类型的新素材，做更清爽的留白构图，重新安排场景、主体和背景关系。`,
    `本张大胆方向：直接生成同类型的新素材，采用对角线、弧形或流动式画面结构，让运动方向和视觉焦点完全不同。`,
    `本张大胆方向：直接生成同类型的新素材，加入新的同类元素组合，保留商业主题但不要沿用原图中的具体元素组合。`,
    `本张大胆方向：直接生成同类型的新素材，改变取景距离和空间层次，重新建立前景、中景、背景。`,
    `本张大胆方向：直接生成同类型的新素材，选择另一个同类场景或同类图案方案，确保看起来不是原图的变体。`,
  ],
}
...
export function buildVariationPrompt(scene: VariationScene, strength: VariationStrength, variationIndex?: number): string {
  ...
  const boldInstruction =
    strength === 'bold'
      ? `
大胆变化特别规则：
1. 把输入图当作“类型参考”和“风格参考”，不要当作需要保留的底图。
2. 直接生成同类型新图片：例如真实沙滩就生成另一张真实沙滩素材，油画花卉就生成另一张油画花卉素材。
3. 不要保留原图的具体构图、具体对象位置、具体物体数量、边缘轮廓、裁切方式或视觉焦点。
4. 除了图片类型、题材类别、媒介风格和色彩气质之外，其余都可以重新生成。`
      : ''
  return `【图片裂变 / 印花再设计】
